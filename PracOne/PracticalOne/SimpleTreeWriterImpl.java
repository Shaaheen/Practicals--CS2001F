...
public class SimpleTreeWriterImpl implements SimpleTreeWriter{
    ...
    public void write(BinaryTreeNode currentTree) {
        BinaryTreeNode[] rootLevel = TreeUtils.createLvlZero(currentTree); //Use TreeUtils method to create a level 0 list
        ...
        for (int y = 0; y < currentTree.getHeight(); y++){ //each level
            ...
            for (int x = 0; x < currentPrintList.size(); x++ ){ //each item
                BinaryTreeNode workingNode = (BinaryTreeNode) currentPrintList.get(x); //retrieves Binary Node at current loop index
                ...
                if (workingNode != null){ //if its not a place holder
                    String itemFormatted = String.format("%-3d",workingNode.getItem()); //format number so the width will always be 3 spaces
